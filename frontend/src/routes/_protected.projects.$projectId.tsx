...
import { useOAuthProviders, useProjects, useProjectSecrets } from '#/hooks/useProjects'
import type { OAuthConfig } from '#/hooks/useProjects'
import { SecretRevealModal } from '#/components/SecretRevealModal'
import type { SecretRevealMode } from '#/components/SecretRevealModal'
import { Trash2, Eye, EyeOff, Plus, X, Copy, Check, Info,  Mail, Globe, Shield, Code2, Github } from 'lucide-react'
...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <Button variant="outline" size="icon" onClick={handleCopy} className="h-8 w-8 text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white bg-slate-100 dark:bg-slate-800 transition-colors">
...
function ProjectConfig() {
  ...
  const { projects, isLoading, updateProjectOauth, updateProjectOrigins, updateProjectEnvironment, updateProjectName, updateProjectFrontendUrl, rotateApiKey, rotateJwtSecret, deleteProject } = useProjects()
  ...
  const handleSaveFrontendUrl = async () => {
    ...
    if (!val) return
    if (!isValidUrl(val)) {
      setFrontendUrlError('Please enter a valid URL starting with http:// or https://')
      return
    }
  ...
  const [revealSecrets, setRevealSecrets] = useState<{ api_key?: string; public_key?: string } | null>(null)
  const [revealMode, setRevealMode] = useState<SecretRevealMode>('api_key_rotated')
  ...
  if (!project) return <div className="text-slate-900 dark:text-white">Project not found</div>
  ...
  const handleProviderSave = async (provider: string, client_id: string, client_secret: string) => {
    const newConfig = { 
      ...oauthConfig, 
      [provider]: { 
        enabled: true, 
  ...
  const handleProviderDisable = async (provider: string) => {
    const newConfig = { ...oauthConfig, [provider]: { enabled: false, client_id: '', client_secret: '' } }
    saveOauthConfig(newConfig)
  }
  ...
  const handleAddOrigin = async () => {
    ...
    if (!isValidUrl(cleanOrigin)) {
      setOriginError('Please enter a valid URL starting with http:// or https://')
      return
    }
    ...
    if (cleanOrigin && !allowedOrigins.includes(cleanOrigin)) {
      ...
      setNewOrigin('')
      await updateProjectOrigins.mutateAsync({ projectId, allowed_origins: newOrigins })
    }
  }
  ...
  const handleToggleEnvironment = async () => {
    const newEnv = project.environment === 'development' ? 'production' : 'development'
    await updateProjectEnvironment.mutateAsync({ projectId, environment: newEnv })
  }
  ...
  const enabledProviders = oauthProviders.filter(p => oauthConfig[p.key].enabled)
  const disabledProviders = oauthProviders.filter(p => !oauthConfig[p.key].enabled)

  const AddProviderModal = () => {
    if (!isAddProviderModalOpen) return null;
    const [selectedProvider, setSelectedProvider] = useState(disabledProviders[0]?.key || '')
      ...
      return (
        <Dialog open={isAddProviderModalOpen} onOpenChange={setIsAddProviderModalOpen}>
          <DialogContent className="sm:max-w-md text-center py-8">
            <p className="text-foreground font-medium mb-4">All available providers are already enabled.</p>
            <Button variant="outline" onClick={() => setIsAddProviderModalOpen(false)}>Close</Button>
          </DialogContent>
        </Dialog>
      )
    }
      ...
      <Dialog open={isAddProviderModalOpen} onOpenChange={setIsAddProviderModalOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="flex justify-end gap-3 pt-2">
            <Button variant="outline" onClick={() => setIsAddProviderModalOpen(false)}>Cancel</Button>
            <Button 
              onClick={() => handleProviderSave(selectedProvider, clientId, clientSecret)} 
  ...
  return (
    ...
    <div className="w-full h-full flex flex-col flex-1 min-h-0 bg-transparent">
              ...
              <div className="space-y-6 animate-in fade-in duration-300">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>Manage your project's identity and environment settings.</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-6">
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2 max-w-xl">
                        <Input
                          ...
                          className="flex-1"
                        />
                        <Button onClick={handleSaveName} disabled={updateProjectName.isPending || editName === project.name}>
                          Save
                        </Button>
                      </div>
                    </div>
                    ...
                    <div className="pt-6 border-t border-border space-y-4">
                      <div>
                        <h3 className="text-sm font-medium text-foreground flex items-center gap-1.5 mb-1">
                          ...
                          <Tooltip>
                            ...
                            <TooltipContent>
                              <p>Production environments have stricter rate limits and security policies.</p>
                            </TooltipContent>
                          </Tooltip>
                        </h3>
                        <p className="text-sm text-muted-foreground mb-4">Toggle between development mode and production mode.</p>
                      </div>
                      <Button
                        variant={project.environment === 'development' ? "default" : "secondary"}
                        onClick={handleToggleEnvironment}
                        disabled={updateProjectEnvironment.isPending}
                        className={project.environment === 'development' 
                      ...
                      >
                        Switch to {project.environment === 'development' ? 'Production' : 'Development'}
                      </Button>
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card className="border-red-200 dark:border-red-900/50 bg-red-50/50 dark:bg-red-900/10">
                  <CardHeader>
                    ...
                    <CardDescription className="text-red-600/80 dark:text-red-400/80">Irreversible destructive actions.</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      <div>
                        <h4 className="font-medium text-slate-900 dark:text-white">Delete Project</h4>
                        <p className="text-sm text-red-600/80 dark:text-red-400/80 mt-1">Permanently delete this project and all its users.</p>
                      </div>
                      <Button variant="destructive" onClick={() => setIsDeleteModalOpen(true)} disabled={deleteProject.isPending}>
              ...
              <Card>
                <CardHeader className="flex flex-row items-center justify-between">
                  <div>
                    ...
                    <CardDescription>Configure which authentication methods your users can use.</CardDescription>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between p-5 border border-border rounded-xl bg-slate-50/50 dark:bg-slate-900/20 transition-colors">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h3 className="font-medium text-foreground">Email & Password</h3>
                          <p className="text-sm text-muted-foreground mt-1">Standard local authentication.</p>
                        </div>
                      </div>
                    ...
                    {enabledProviders.map((provider) => {
                      return (
                        <div key={provider.key} className="flex items-center justify-between p-5 border border-border rounded-xl bg-card hover:bg-accent/50 transition-colors">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-muted-foreground mt-1">OAuth integration is active.</p>
                            </div>
                          </div>
                          <Button variant="destructive" size="sm" onClick={() => handleProviderDisable(provider.key)}>
                            Remove
                          </Button>
                        </div>
                      )
            ...
            <div className="space-y-8 animate-in fade-in duration-300">
              ...
              <Card>
                <CardHeader className="flex flex-row items-start gap-4 space-y-0">
                  ...
                  <div className="flex-1">
                    <CardTitle className="text-lg">Frontend App URL</CardTitle>
                    <CardDescription className="mt-1.5">
                      This is the primary URL where your frontend application is hosted. We use this to redirect your users back to your app after they successfully log in via OAuth providers.
                    </CardDescription>
                  </div>
                </CardHeader>
                <CardContent className="pl-18">
                  <div className="flex flex-col sm:flex-row gap-3 max-w-2xl">
                    <Input
                      ...
                      onKeyDown={e => e.key === 'Enter' && handleSaveFrontendUrl()}
                      className="flex-1"
                    />
                    <Button
                      onClick={handleSaveFrontendUrl}
                      disabled={updateProjectFrontendUrl.isPending || frontendUrl.trim() === project.frontend_url}
                      className="shrink-0"
                    >
                      Save Changes
                    </Button>
                  ...
                  {frontendUrlError && (
                    <p className="text-sm text-destructive font-medium mt-2">{frontendUrlError}</p>
                  )}
                </CardContent>
              </Card>
              ...
              <Card>
                <CardHeader className="flex flex-row items-start gap-4 space-y-0">
                  ...
                  <div className="flex-1">
                    <CardTitle className="text-lg">Allowed CORS Origins</CardTitle>
                    <CardDescription className="mt-1.5">
                      Whitelist specific domains that are permitted to make API requests to your Cerberus project. Requests from domains not listed here will be blocked by the browser. You can add up to 5 origins.
                    </CardDescription>
                  </div>
                </CardHeader>
                <CardContent className="pl-18 space-y-6">
                  <div className="flex flex-col sm:flex-row gap-3 max-w-2xl">
                    ...
                    <Button
                      ...
                      variant="secondary"
                      disabled={allowedOrigins.length >= 5 || !newOrigin.trim()}
                      className="shrink-0"
                    >
                      Add Origin
                    </Button>
                  ...
                  {originError && (
                    <p className="text-sm text-destructive font-medium mt-2">{originError}</p>
                  )}
                  ...
                  {allowedOrigins.length > 0 ? (
                          ...
                          <ContextMenu key={origin}>
                            <ContextMenuTrigger>
                              <li className="flex items-center justify-between p-3.5 bg-card hover:bg-muted/30 transition-colors">
                                <span className="text-sm font-mono cursor-default text-foreground/90">{origin}</span>
                                <Button
                                  variant="ghost"
                                  size="icon"
                    ...
                    <div className="p-8 border border-dashed border-border rounded-xl text-center max-w-2xl bg-muted/20">
                      ...
                      <p className="text-sm font-medium text-foreground">No origins whitelisted</p>
                      <p className="text-sm text-muted-foreground mt-1">External API requests from browsers will be blocked until you add an origin.</p>
                    </div>
                  )}
                </CardContent>
              </Card>
              ...
              <Card>
                <CardHeader className="flex flex-row items-center justify-between">
                  <div>
                    ...
                    <CardDescription>Used to scope SDK requests to this project via <code className="text-xs bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded">X-Cerberus-API-Key</code></CardDescription>
                  </div>
                  <Button onClick={() => setIsRotateApiModalOpen(true)} variant="outline" size="sm" className="h-7 px-3 text-xs font-medium bg-amber-500/10 text-amber-700 dark:text-amber-500 hover:bg-amber-500/20 border-amber-500/20 transition-colors">
                    Rotate Key
                  </Button>
                </CardHeader>
                <CardContent>
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-2 p-3 bg-slate-50 dark:bg-black/40 border border-slate-200 dark:border-slate-800 rounded-lg font-mono text-sm text-slate-500 dark:text-slate-400 select-none">
                      <span className="text-slate-700 dark:text-slate-300">cerb_</span>
                      <span className="tracking-widest">••••••••••••••••••••••••••••</span>
                    </div>
                    <p className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
                      <Info className="w-3.5 h-3.5 shrink-0 text-blue-400" />
                      The plaintext API key is only shown <strong>once</strong> upon creation or rotation and cannot be retrieved again. Rotate the key if you've lost access to it.
                    </p>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardHeader className="flex flex-row items-center justify-between">
                  <div>
                    ...
                    <CardDescription>Use this on your backend to verify the signature of JWTs issued to your end-users.</CardDescription>
                  </div>
                  <Button onClick={() => setIsRotateJwtModalOpen(true)} variant="outline" size="sm" className="h-7 px-3 text-xs font-medium bg-amber-500/10 text-amber-700 dark:text-amber-500 hover:bg-amber-500/20 border-amber-500/20 transition-colors">
                    Rotate Key Pair
                  </Button>
                </CardHeader>
                <CardContent>
                      ...
                      <div className="flex gap-2 items-start relative">
                        <ScrollArea className="w-full h-35 bg-slate-50 dark:bg-black/50 border border-slate-200 dark:border-slate-800 rounded-lg">
                          <pre className="p-4 text-slate-700 dark:text-slate-300 font-mono text-xs whitespace-pre">
                            {showJwt ? secrets.public_key : '-----BEGIN PUBLIC KEY-----\n••••••••••••••••••••••••••••••••••••••••••••••••••••••••••\n••••••••••••••••••••••••••••••••••••••••••••••••••••••••••\n••••••••••••••••••••••••••••••••••••••••••••••••••••••••••\n••••••••••••••••••••••••••••••••••••••••••••••••••••••••••\n-----END PUBLIC KEY-----'}
                          </pre>
                          <ScrollBar orientation="horizontal" />
                        </ScrollArea>
                        <div className="absolute top-2 right-2 flex flex-col gap-2">
                          <Button onClick={() => setShowJwt(!showJwt)} variant="outline" size="icon" className="h-8 w-8 text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white bg-slate-100 dark:bg-slate-800 transition-colors">
                            {showJwt ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                          </Button>
                          <CopyButton text={secrets.public_key} />
                        </div>
                      </div>
                      <p className="text-xs text-slate-500">Unlike the API key, the public key is safe to retrieve at any time — it is not a secret.</p>
                    </div>
                  ) : (
      ...
      <AlertDialog open={isRotateApiModalOpen} onOpenChange={setIsRotateApiModalOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Rotate API Key</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure? This will instantly invalidate your current API key. Existing apps using the old key will break.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={rotateApiKey.isPending}>Cancel</AlertDialogCancel>
            <AlertDialogAction 
              onClick={(e) => { e.preventDefault(); doRotateApiKey(); }} 
      ...
      <AlertDialog open={isRotateJwtModalOpen} onOpenChange={setIsRotateJwtModalOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Rotate JWT Secret</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure? This generates new RSA keys. All currently issued JWTs will fail signature validation on your backend until they expire.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={rotateJwtSecret.isPending}>Cancel</AlertDialogCancel>
            <AlertDialogAction 
              onClick={(e) => { e.preventDefault(); doRotateJwtSecret(); }} 
              disabled={rotateJwtSecret.isPending}
              className="bg-amber-600 text-white hover:bg-amber-700 focus:ring-amber-600"
            >
              {rotateJwtSecret.isPending ? 'Rotating...' : 'Yes, rotate JWT keys'}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <AlertDialog open={isDeleteModalOpen} onOpenChange={(open) => {
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to permanently delete project <span className="font-bold">{project.name}</span>? All data and users will be lost forever.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <div className="py-4">
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction 
              onClick={(e) => { e.preventDefault(); doDeleteProject(); }} 
