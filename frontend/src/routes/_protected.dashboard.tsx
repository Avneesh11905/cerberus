    ...
    <div className="flex flex-col h-full bg-transparent">
      ...
      <div className="max-w-6xl w-full mx-auto p-8">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Manage your Cerberus Identity projects.</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center p-12 border border-slate-200 dark:border-slate-800 border-dashed rounded-2xl bg-slate-50 dark:bg-[#0a0a0a] text-center">
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-2 max-w-sm mb-6">
            Get started by creating your first project to manage authentications, API keys, and settings.
          </p>
          <Button onClick={() => setIsCreateOpen(true)}>
            <Plus className="w-4 h-4 mr-2" />
                  ...
                  <div className="relative z-10">
                    ...
                    <div className="mt-6 flex items-center justify-between text-xs font-medium">
                      ...
                      {project.environment === 'development' ? (
                        <span className="bg-amber-500/10 text-amber-500 px-2.5 py-1 rounded-md border border-amber-500/20">Dev</span>
                      ) : (
                        <span className="bg-emerald-500/10 text-emerald-500 px-2.5 py-1 rounded-md border border-emerald-500/20">Prod</span>
                      )}
                    </div>
                  </div>
                </Link>
      ...
      <AlertDialog open={!!projectToDelete} onOpenChange={(open) => !open && setProjectToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to permanently delete project <span className="font-bold">{projectToDelete?.name}</span>? All data and users will be lost forever.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
