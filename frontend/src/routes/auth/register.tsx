...
function Register() {
  ...
  const onOtpSubmit = async (values: z.infer<typeof otpSchema>) => {
    setError('')
    try {
      await verifyEmail({ email: registerForm.getValues("email"), otp: values.otp })
      navigate({ to: '/dashboard' })
    } catch (err: any) {
  ...
  const handleGoogleLogin = () => window.location.href = `${API_URL}/auth/login/google`
  const handleGithubLogin = () => window.location.href = `${API_URL}/auth/login/github`
    ...
    <div className="flex min-h-screen bg-white dark:bg-[#0a0a0a]">
      ...
      <div className="hidden lg:flex w-1/2 flex-col justify-between bg-slate-900 p-12 text-white border-r border-slate-800">
        ...
        <div className="max-w-md">
          <h1 className="text-4xl font-bold tracking-tight mb-4">Start your journey.</h1>
          <p className="text-slate-400 text-lg leading-relaxed">
            Create an account to integrate advanced authentication and centralized management into your products in minutes.
          </p>
        </div>
        ...
        <div className="w-full max-w-[400px]">
          <div className="flex flex-col space-y-2 mb-8">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              {step === 'register' ? 'Enter your details below to get started.' : `We sent an OTP to ${registerForm.getValues("email")}`}
            </p>
          </div>
            ...
            <>
              <Form {...registerForm}>
                <form onSubmit={registerForm.handleSubmit(onRegisterSubmit)} className="space-y-5">
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-sm font-medium text-slate-900 dark:text-slate-200">Password</FormLabel>
                        <FormControl>
                          <Input type="password" placeholder="Minimum 8 characters" className="h-11 bg-transparent border-slate-300 dark:border-slate-800 focus-visible:ring-slate-400 dark:focus-visible:ring-slate-700 rounded-lg shadow-sm" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  >
                    {registerForm.formState.isSubmitting ? "Creating account..." : "Start for free"}
                  </Button>
                </form>
              </Form>
              ...
              <div className="relative my-8">
                ...
                <div className="relative flex justify-center text-xs uppercase">
                  <span className="px-4 bg-white dark:bg-[#0a0a0a] text-slate-500 font-medium tracking-wider">Or sign up with</span>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <Button type="button" variant="outline" onClick={handleGoogleLogin} className="h-11 bg-transparent border-slate-300 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-900 rounded-lg font-medium text-slate-700 dark:text-slate-300 shadow-sm transition-colors">
                ...
                <Button type="button" variant="outline" onClick={handleGithubLogin} className="h-11 bg-transparent border-slate-300 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-900 rounded-lg font-medium text-slate-700 dark:text-slate-300 shadow-sm transition-colors">
            ...
            <Form {...otpForm}>
              <form onSubmit={otpForm.handleSubmit(onOtpSubmit)} className="space-y-5">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-sm font-medium text-slate-900 dark:text-slate-200">6-Digit Verification Code</FormLabel>
                      <FormControl>
                        <Input 
                          placeholder="000000" 
                          maxLength={6}
                ...
                <Button
                  ...
                  className="w-full h-11 bg-slate-900 hover:bg-slate-800 text-white dark:bg-white dark:hover:bg-slate-200 dark:text-slate-900 font-medium rounded-lg shadow-sm transition-colors mt-2"
                >
                  {otpForm.formState.isSubmitting ? "Verifying..." : "Verify & Enter"}
                </Button>
              </form>
            </Form>
          )}
