...
function getLevelBadge(level: string) {
  const base = "inline-flex items-center px-2 py-0.5 rounded text-xs font-bold uppercase "
...
function SuperadminLogsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => getSystemLogs(page, 100, levelFilter === 'ALL' ? undefined : levelFilter),
    ...
    <div className="space-y-6 pt-4">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        <div>
          ...
          <p className="text-sm font-semibold text-slate/60 mt-1">Real-time audit and error logs for the platform.</p>
        </div>
        ...
        <Table>
          <TableHeader className="bg-sand/30">
            <TableRow className="border-taupe/30 hover:bg-transparent">
              <TableHead className="font-bold text-slate w-[180px]">Timestamp</TableHead>
              <TableHead className="font-bold text-slate w-[100px]">Level</TableHead>
              <TableHead className="font-bold text-slate w-[250px]">Source</TableHead>
            ...
            {isLoading ? (
              Array.from({ length: 15 }).map((_, i) => (
                <TableRow key={i} className="border-taupe/20 hover:bg-transparent">
                  <TableCell><Skeleton className="h-4 w-32 bg-taupe/20" /></TableCell>
                  <TableCell><Skeleton className="h-5 w-16 bg-taupe/20 rounded" /></TableCell>
                  <TableCell><Skeleton className="h-4 w-40 bg-taupe/20" /></TableCell>
                  <TableCell><Skeleton className="h-4 w-64 bg-taupe/20" /></TableCell>
                </TableRow>
              ))
            ) : data?.items.length === 0 ? (
              <TableRow className="hover:bg-transparent font-sans">
                <TableCell colSpan={4} className="h-32 text-center text-slate/60 font-semibold">
                  No logs found.
                </TableCell>
              </TableRow>
            ) : (
              data?.items.map((log) => (
                <TableRow key={log.id} className="border-taupe/20 hover:bg-sand/30 transition-colors">
                  ...
                  <TableCell className="text-slate font-medium align-top pt-4 break-all" title={log.source}>
                    {log.source}
                  </TableCell>
                  <TableCell className="text-slate/70 text-xs whitespace-pre-wrap break-words align-top pt-4 py-4" title="Log Message">
                    {log.file && (
