...
function DocsLayout() {
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden bg-slate-50 dark:bg-[#0a0a0a] text-slate-900 dark:text-slate-50 selection:bg-blue-200 selection:text-blue-900 dark:selection:bg-blue-800 dark:selection:text-white">
      ...
      <nav className="shrink-0 w-full z-50 flex items-center justify-between px-6 py-4 bg-white dark:bg-[#0a0a0a] border-b border-slate-200 dark:border-white/5">
        <div className="flex items-center gap-8">
          <Link to="/" className="flex items-center gap-3 select-none hover:opacity-80 transition-opacity">
            <img src="/logo.webp" alt="Cerberus" className="w-12 h-12 select-none" draggable={false} />
            <span className="text-2xl tracking-tight" style={{ fontFamily: 'Audiowide, sans-serif' }}>Cerberus</span>
          </Link>
        </div>
        
        <div className="flex items-center gap-4 sm:gap-6">
          <Button variant="ghost" size="sm" asChild className="hidden md:flex gap-2 text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white">
            <Link to="/dashboard">
          ...
          {isLoading ? (
            ...
            <div className="flex items-center gap-4">
              <Link to="/auth/login" className="hidden sm:block text-sm font-medium text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white transition-colors">
                Log in
              </Link>
              <Link to="/auth/register" className="h-9 inline-flex items-center justify-center rounded-xl bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700 transition-colors shadow-sm">
                Get API Key
              </Link>
            </div>
          )}
      ...
      {/* Main Content Area */}
      <div ref={scrollRef} className="flex-1 min-h-0 w-full relative overflow-y-auto custom-scrollbar id-docs-scroll-container">
          ...
          <div className="mb-8">
            <h4 className="text-xs font-semibold text-slate-900 dark:text-white uppercase tracking-wider mb-4 font-outfit">Documentation</h4>
            <div className="flex flex-col gap-2">
              <Link 
                to="/docs"
                activeProps={{ className: 'text-blue-600 dark:text-blue-400 font-medium' }}
                inactiveProps={{ className: 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white' }}
          ...
          <div className="mt-12 mb-8 text-xs text-slate-500 dark:text-slate-400">
            &copy; {new Date().getFullYear()} Avneesh Mahajan.<br/> Proprietary Software.
          </div>
        </aside>
