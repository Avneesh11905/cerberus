        ...
        <div className="prose prose-slate dark:prose-invert max-w-none">
          <section id="user-management" className="scroll-mt-8 mb-16">
            ...
            <p className="text-lg text-slate-600 dark:text-slate-400 mb-4 leading-relaxed">
              The <code>cerberus.users</code> module caches user data as the
              single source of truth and automatically updates it during
              workflows.
            </p>
            ...
            <p className="text-slate-600 dark:text-slate-400 text-base mb-4">
              The SDK automatically caches the user profile. You never need to
              worry about manually re-fetching the user.
            </p>
            <CodeBlock
              language="typescript"
...
const freshUser = await cerberus.users.getMe(true);`}
              ...
              args={[
                {
                  ...
                  type: 'boolean?',
                  defaultValue: 'false',
                  desc: 'Optional. If true, bypasses the cache and forces a fresh network fetch.',
                },
              ]}
              ret="Promise<User>"
            />
            ...
            <p className="text-slate-600 dark:text-slate-400 text-base mb-4">
              You can update the user profile, and the SDK will instantly sync
              the updated state with the cache.
            </p>
            <CodeBlock
              language="typescript"
              ...
              args={[
                {
                  name: 'data.name',
                  type: 'string?',
                  desc: 'Optional new display name for the user.',
                },
                {
                  name: 'data.picture',
                  type: 'string?',
                  desc: 'Optional new avatar URL.',
                },
                {
                  name: 'data.receive_updates',
                  type: 'boolean?',
                  desc: 'Optional preference for marketing/product emails.',
                },
              ]}
              ret="Promise<User>"
            />
