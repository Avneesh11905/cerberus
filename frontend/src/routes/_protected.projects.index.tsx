...
function ProjectsIndexPage() {
  ...
  const handleDownloadApiKey = () => {
    ...
    const dataStr =
      ...
      encodeURIComponent(
        JSON.stringify({ api_key: createdCredentials.api_key }, null, 2),
      )
    const downloadAnchorNode = document.createElement('a')
    downloadAnchorNode.setAttribute('href', dataStr)
    downloadAnchorNode.setAttribute(
      'download',
      `${createdCredentials.name.replace(/\s+/g, '_').toLowerCase()}_api_key.json`,
    )
  ...
  const handleDownloadPublicKey = () => {
    ...
    const dataStr =
      ...
      encodeURIComponent(createdCredentials.public_key)
    const downloadAnchorNode = document.createElement('a')
    downloadAnchorNode.setAttribute('href', dataStr)
    downloadAnchorNode.setAttribute(
      'download',
      `${createdCredentials.name.replace(/\s+/g, '_').toLowerCase()}_public_key.pem`,
    )
    document.body.appendChild(downloadAnchorNode)
    downloadAnchorNode.click()
    downloadAnchorNode.remove()
          ...
          <div className="max-w-7xl mx-auto flex flex-col gap-8 w-full">
                  ...
                  {createdCredentials ? (
                    <div className="space-y-6">
                      <DialogHeader>
                        ...
                        <DialogDescription className="text-terracotta font-bold flex items-start gap-2 mt-2 bg-terracotta/10 p-3 border-2 border-terracotta rounded-xl">
                          ...
                          Warning: This is the ONLY time you will see this API
                          Key. Please copy it or download the JSON file now. If
                          you lose it, you will need to rotate the key.
                        </DialogDescription>
                      </DialogHeader>
                          ...
                          <div className="relative">
                            <pre className="w-full min-h-30 border-2 border-slate bg-taupe/10 px-4 py-4 text-xs font-mono rounded-xl overflow-hidden whitespace-pre-wrap break-all leading-relaxed">
                              {showPublicKey
                                ...
                                : createdCredentials.public_key.replace(
                                    /(?<=-----BEGIN PUBLIC KEY-----\n)[\s\S]*?(?=\n-----END PUBLIC KEY-----)/,
                                    '****************************************************************\n****************************************************************\n****************************************************************\n****************************************************************',
                                  )}
                            </pre>
                    ...
                    <form onSubmit={handleCreateProject}>
                      <DialogHeader>
                        ...
                        <DialogDescription>
                          Enter a name for your new Cerberus integration
                          project.
                        </DialogDescription>
                      </DialogHeader>
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center p-12 border-2 border-dashed border-taupe bg-sand text-center">
                ...
                <p className="text-slate/70 mb-6 font-semibold max-w-sm">
                  Create your first project to start authenticating users and
                        ...
                        <Card className="relative overflow-hidden flex flex-col justify-between h-full hover:bg-sand/80 transition-all duration-300 ease-out group-hover:-translate-y-1 group-hover:shadow-[4px_4px_0px_0px_var(--taupe)]">
                          <CardHeader className="pb-4">
                            <div className="flex justify-between items-start mb-2">
                              <div className="flex items-center gap-1.5 px-2.5 py-0.5 rounded-none border-2 border-slate text-xs font-bold uppercase bg-vanilla text-slate">
                                <div
                                  className={`w-2 h-2 rounded-full ${project.environment === 'production' ? 'bg-sage' : 'bg-ochre'}`}
                                />
                                {project.environment}
                              </div>
                            </div>
                            <CardTitle className="text-2xl mt-2 line-clamp-1 transition-colors group-hover:text-sage">
                              {project.name}
                            </CardTitle>
                          </CardHeader>
                          ...
                          <CardContent className="flex flex-col gap-3 py-4">
                            <div className="flex items-center gap-3 text-sm font-semibold text-slate/80">
                              ...
                              {
                                Object.values(project.oauth_config).filter(
                                  (c) => c.enabled,
                                ).length
                              }{' '}
                              Auth Providers
                            ...
                            <div className="flex items-center gap-3 text-sm font-semibold text-slate/80">
                              ...
                              {project.allowed_origins.length} Allowed Origins
                            </div>
                            <div className="flex items-center gap-3 text-sm font-semibold text-slate/80">
            ...
            {total > size && (
              <div className="flex items-center justify-between mt-8">
                <p className="text-sm font-medium text-slate/70">
                  ...
                  {Math.min(page * size, total)} of {total} projects
                </p>
              ...
              <DialogContent className="sm:max-w-md border-terracotta">
                <DialogHeader>
                  ...
                  <DialogDescription className="mt-2">
                    ...
                    <strong className="text-slate font-bold">
                      {projectToDelete?.name}
                    </strong>
                    ? This action cannot be undone and will permanently destroy
                    the project and all its users.
                  </DialogDescription>
                </DialogHeader>
