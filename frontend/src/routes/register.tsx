...
function RegisterPage() {
  ...
  const [pendingAction, setPendingAction] = useState<'register' | 'google' | 'github' | null>(null)
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-1.5">
            ...
            <div className="relative">
              <Input id="password" type={showPassword ? "text" : "password"} placeholder="Min. 8 chars" className="pr-10" {...register('password')} />
              <button
                type="button"
            ...
            {errors.password && <p className="text-terracotta text-xs font-medium">{errors.password.message}</p>}
          </div>

          <div className="space-y-1.5">
            <Label htmlFor="confirmPassword">Confirm Password</Label>
            <div className="relative">
              <Input id="confirmPassword" type={showPassword ? "text" : "password"} placeholder="Confirm" className="pr-10" {...register('confirmPassword')} />
              <button
                type="button"
                onClick={() => setShowPassword(!showPassword)}
                className="absolute right-3 top-1/2 -translate-y-1/2 text-slate/50 hover:text-slate transition-colors"
            ...
            {errors.confirmPassword && <p className="text-terracotta text-xs font-medium">{errors.confirmPassword.message}</p>}
          </div>
        </div>
        ...
        {authError && (
          <div className="p-3 bg-terracotta/10 border border-terracotta/20 rounded-md">
            <p className="text-terracotta text-sm font-bold text-center">{authError}</p>
          </div>
        )}
        ...
        <div className="relative my-4">
          ...
          <div className="relative flex justify-center text-xs uppercase">
            <span className="bg-sand px-2 text-slate font-bold">Or continue with</span>
          </div>
        </div>
          ...
          >
            {pendingAction === 'google' ? (
              ...
              <>
                ...
                <span className="font-semibold text-[13px] tracking-wide">Google</span>
              </>
            )}
          </Button>
          <Button 
          ...
          >
            {pendingAction === 'github' ? (
              ...
              <>
                ...
                <span className="font-semibold text-[13px] tracking-wide">GitHub</span>
              </>
            )}
          </Button>
        </div>
