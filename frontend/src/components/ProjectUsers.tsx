...
import { getProjectUsers, updateProjectUserStatus, getProjectUserClaims, updateProjectUserClaims, type ProjectUser } from '../api/projects'
...
export function ProjectUsers({ projectId }: { projectId: string }) {
  ...
  const openClaims = async (user: ProjectUser) => {
    ...
    try {
      ...
      // If it's { overrides: {} }, use that. Otherwise use raw data.
      const overrides = data.overrides || data || {}
      setClaims(Object.entries(overrides).map(([k, v]) => ({ key: k, value: String(v) })))
    } catch (err) {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Manage users who have authenticated with this project.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="border-2 border-slate">
          {loading ? (
            ...
            <div className="p-12 flex flex-col items-center justify-center text-center bg-sand">
              ...
              <h3 className="text-xl font-bold text-slate mb-2">No Users Found</h3>
              <p className="text-slate/70 font-semibold max-w-sm">No users match your criteria or no users have authenticated yet.</p>
            </div>
          ) : (
                    ...
                    <tr key={user.id} className="hover:bg-sand/30 transition-colors">
                      ...
                      <td className="px-4 py-4">
                        {user.is_active ? (
                          ...
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-bold bg-terracotta/20 text-terracotta border border-terracotta/30">
                            <span className="w-1.5 h-1.5 rounded-full bg-terracotta" /> Inactive
                          </span>
                        )}
                      </td>
                      ...
                      <td className="px-4 py-4 text-right">
                        <div className="flex justify-end gap-2">
                          <Button variant="outline" size="sm" className="h-8 text-xs font-bold" onClick={() => toggleStatus(user)}>
                            {user.is_active ? 'Deactivate' : 'Activate'}
                          </Button>
                          <Button variant="outline" size="sm" className="h-8 text-xs font-bold gap-1" onClick={() => openClaims(user)}>
        ...
        <div className="flex items-center justify-between">
          <p className="text-sm font-semibold text-slate/70">
            Showing {users.length > 0 ? (page - 1) * size + 1 : 0} to {Math.min(page * size, total)} of {total}
          </p>
      ...
      <Dialog open={!!selectedUser} onOpenChange={(open: boolean) => !open && setSelectedUser(null)}>
        <DialogContent className="sm:max-w-150 border-2 border-slate bg-vanilla p-0 overflow-hidden shadow-[8px_8px_0px_rgba(30,41,59,1)]">
          <DialogHeader className="p-6 bg-sand border-b-2 border-slate">
            <DialogTitle className="text-2xl font-black text-slate">User Claim Overrides</DialogTitle>
          ...
          <DialogFooter className="p-6 bg-sand border-t-2 border-slate flex justify-end">
             ...
             <Button onClick={handleSaveClaims} disabled={savingClaims}>
               {savingClaims ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : null}
               Save Overrides
             </Button>
          </DialogFooter>
        </DialogContent>
