  ...
  processLiveEvent: (event: any) =>
    set((state) => {
      ...
      if (et === 'PROJECT_CREATED') {
        if (newTotals.projects_created !== undefined)
          newTotals.projects_created += 1
      ...
      if (newData.platform_adoption) {
        ...
        if (et === 'LOGIN_SUCCESS')
          pa.login_successes = (pa.login_successes ?? 0) + 1
        if (et === 'LOGIN_FAILED')
          pa.login_failures = (pa.login_failures ?? 0) + 1
        if (et === 'REGISTRATION')
          pa.registrations = (pa.registrations ?? 0) + 1
        if (et === 'TENANT_ONBOARDED')
          pa.total_tenants = (pa.total_tenants ?? 0) + 1
        newData.platform_adoption = pa
      }
      if (newData.end_user_usage) {
        const eu = { ...newData.end_user_usage }
        if (et === 'PROJECT_CREATED')
          eu.total_projects = (eu.total_projects ?? 0) + 1
        if (et === 'REGISTRATION')
          eu.registrations = (eu.registrations ?? 0) + 1
          ...
          onmessage: (event) => {
            try {
              ...
              if (parsed.metrics && Array.isArray(parsed.metrics)) {
                useAnalyticsStream
                  .getState()
                  .processBulkData(parsed.metrics, parsed.totals)
              }
              ...
              if (parsed.platform_adoption) {
                ...
                useAnalyticsStream.getState().setData({
                  ...
                  totals: {
                    api_requests:
                      (parsed.platform_adoption?.api_requests ?? 0) +
                      (parsed.end_user_usage?.api_requests ?? 0),
                    login_successes:
                      (parsed.platform_adoption?.login_successes ?? 0) +
                      (parsed.end_user_usage?.login_successes ?? 0),
                    login_failures:
                      (parsed.platform_adoption?.login_failures ?? 0) +
                      (parsed.end_user_usage?.login_failures ?? 0),
