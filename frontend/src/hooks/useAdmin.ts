...
export function useAdmin() {
  ...
  const updateTenantStatus = useMutation({
    mutationFn: async ({ tenantId, is_active }: { tenantId: string; is_active: boolean }) => {
      const res = await api.patch<User>(`/admin/tenants/${tenantId}/status`, { is_active });
      return res.data;
    },
    onSuccess: () => {
...
export function useAdminLogs(
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await api.get<SystemLogRes[]>(`/admin/logs?${params.toString()}`);
      return res.data;
    },
