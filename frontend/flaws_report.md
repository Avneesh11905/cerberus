...
*   **Reactive vs. Proactive Rotation:** The `beforeLoad` hook in `_protected.tsx` only checked if `accessToken` *existed* in local storage, not if it was *valid*. 
    ...
    *   **The Flaw:** While this worked mechanically, it allowed the user to see a flashing page or loading skeletons *before* the refresh happened.
    *   **Resolution:** Created a fast, client-side JWT decoder (`src/lib/jwt.ts`) that extracts the `exp` claim without needing the RSA public key. The `beforeLoad` hook in `_protected.tsx` now proactively checks if the token is within 10 seconds of expiring. If it is, it explicitly halts the router, hits `/auth/refresh` itself, updates the Zustand store, and *then* allows the navigation to proceed silently, completely eliminating the UI flicker and redundant 401 API failures.

## 5. Superadmin Pages Accessible by Demoted Tenants - ✅ [RESOLVED]
*   **The Stale Role Problem:** Because the user object is persisted in `localStorage` (via Zustand) and was *never* updated during background token rotation (fixed in Issue 3), the frontend's concept of the user's role could become stale.
