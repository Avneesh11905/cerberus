...
def upgrade() -> None:
    ...
    for row in projects:
            ...
            for provider, config in oauth_config_dict.items():
                if "client_secret" in config:
                    ...
                    if secret and not secret.startswith("gAAAAAB"):
                        config["client_secret"] = fernet.encrypt(
                            secret.encode()
                        ).decode()
                        update_needed = True
        ...
        if update_needed:
            oauth_config_val = (
                json.dumps(new_oauth_config) if new_oauth_config else None
            )
            conn.execute(
                sa.text(
                    "UPDATE projects SET private_key = :pk, oauth_config = cast(:oc as jsonb) WHERE id = :pid"
                ),
            ...
            for provider, config in oauth_config_dict.items():
                if "client_secret" in config:
                    ...
                    if secret and secret.startswith("gAAAAAB"):
                        config["client_secret"] = fernet.decrypt(
                            secret.encode()
                        ).decode()
                        update_needed = True
                new_oauth_config[provider] = config
