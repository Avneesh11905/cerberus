...
class OAuthSettings(_BaseSettings):
    ...
    def get_credentials(self, provider: str) -> tuple[str, str]:
        ...
        keys_to_check = [
            ...
            f"{provider.lower()}_client_id",
            f"{provider.upper()}_CLIENT_SECRET",
            f"{provider.lower()}_client_secret",
        ]

        # We can dynamically access extra fields added via .env because of `extra="allow"`
        client_id = getattr(
            self, keys_to_check[0], getattr(self, keys_to_check[1], None)
        )
        client_secret = getattr(
            self, keys_to_check[2], getattr(self, keys_to_check[3], None)
        )
