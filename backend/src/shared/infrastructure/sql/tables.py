...
class Project(Base):
    ...
    tenant_id: Mapped[UUID] = mapped_column(Uuid, ForeignKey("users.id", ondelete="CASCADE"), index=True)
    ...
    oauth_config: Mapped[dict] = mapped_column(JSON_TYPE, server_default=text("'{}'"), nullable=False)
    allowed_origins: Mapped[list[str]] = mapped_column(JSON_TYPE, server_default=text("'[]'"), nullable=False)
    environment: Mapped[str] = mapped_column(String, server_default=text("'development'"), nullable=False)
    ...
    tenant: Mapped["User"] = relationship(back_populates="owned_projects", foreign_keys=[tenant_id])
    end_users: Mapped[list["User"]] = relationship(back_populates="project", foreign_keys="User.project_id", cascade="all, delete-orphan")
...
class User(Base):
    ...
    role: Mapped[UserRole] = mapped_column(SQLEnum(UserRole, native_enum=False, length=20), server_default=text("'user'"), nullable=False)
    ...
    owned_projects: Mapped[list["Project"]] = relationship(
        back_populates="tenant", foreign_keys=[Project.tenant_id], cascade="all, delete-orphan"
    )
    ...
    __table_args__ = (
        Index("uq_tenant_email", "email", postgresql_where=text("project_id IS NULL"), unique=True),
        Index("uq_project_email", "email", "project_id", postgresql_where=text("project_id IS NOT NULL"), unique=True),
    )
...
class UserOAuthAccount(Base):
    ...
    __table_args__ = (
        Index("uq_tenant_provider_oauth_sub", "provider", "oauth_sub", postgresql_where=text("project_id IS NULL"), unique=True),
        Index("uq_project_provider_oauth_sub", "provider", "oauth_sub", "project_id", postgresql_where=text("project_id IS NOT NULL"), unique=True),
    )
