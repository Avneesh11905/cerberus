...
RESERVED_KEYS = {"sub","email","role","exp","iat","jti",
...
class UpdateUserClaimsUseCase[SessionType](BaseProjectUseCase[SessionType]):
    ...
    async def execute(self, session: SessionType, project_id, tenant_id, user_id, overrides) -> UserProfile:
        ...
        if unknown:
            raise ValueError(f"Keys not defined in project schema cannot be used: {unknown}")
