    ...
    async def execute(
        ...
        async with self.uow:
            ...
            logs = await self.uow.log_repo.get_recent_logs(
                skip=skip, limit=limit, level=parsed_level, start_date=start_date, end_date=end_date
            )
            total = await self.uow.log_repo.count_logs(level=parsed_level, start_date=start_date, end_date=end_date)
            return logs, total
