...
from src.modules.auth.authentication.infrastructure.oauth import PARSERS, PROVIDERS
...
class OAuthServiceAdapter(OAuthServicePort[AsyncSession, Request]):
    def __init__(
        self,
        project_query_repo: ProjectQueryRepositoryPort[AsyncSession],
    ...
    async def get_authorization_url(
        ...
        client = await self._get_client(provider, project_id, session)
    ...
    async def _get_client(
        self, provider: str, project_id: UUID | None, session: AsyncSession
    ):
        if not project_id:
