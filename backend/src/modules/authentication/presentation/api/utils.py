...
def set_refresh_token_cookie(response: Response, refresh_token: str) -> None:
    ...
    response.set_cookie(
        key="refresh_token",
        value=refresh_token,
        httponly=get_settings().cookie.HTTP_ONLY,
        secure=get_settings().cookie.SECURE,
        samesite=get_settings().cookie.SAMESITE,  # type: ignore
        max_age=get_settings().token.REFRESH_TOKEN_LIFETIME_DAYS * 86400,
        path=get_settings().cookie.PATH,
    )
...
async def build_auth_redirect_async(
    ...
    """
    base_url = (
        frontend_url if frontend_url else get_settings().url.FRONTEND_URL
    ).rstrip("/")
