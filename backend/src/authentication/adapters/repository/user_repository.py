...
class SQLUserRepositoryAdapter(UserRepositoryPort[AsyncSession]):
    ...
    async def find_by_oauth(self, session: AsyncSession, provider: str, oauth_sub: str, project_id: UUID | None = None) -> UserIdentity | None:
        """Look up a user by their OAuth provider + subject ID within a project."""
        
        stmt = select(User).join(UserOAuthAccount).where(
    ...
    async def find_by_email(self, session: AsyncSession, email: str, project_id: UUID | None = None) -> UserIdentity | None:
    ...
    async def create_user_with_oauth(
        ...
        provider: str, oauth_sub: str, project_id: UUID | None = None, role: str = "user"
    ) -> UserIdentity:
        """Create a new user and link an OAuth account."""
        user = User(email=email, name=name, picture=picture, is_verified=True, project_id=project_id, role=role)
        session.add(user)
        await session.flush()
    ...
    async def link_oauth_account(
        self, session: AsyncSession, user_id: UUID, provider: str, oauth_sub: str, project_id: UUID | None = None
    ) -> None:
    ...
    async def create_user_with_password(
        ...
        project_id: UUID | None = None, role: str = "user"
    ) -> UserIdentity:
        """Create a new user and store their local password."""
        user = User(email=email, name=name, is_verified=is_verified, project_id=project_id, role=role)
        session.add(user)
        await session.flush()
