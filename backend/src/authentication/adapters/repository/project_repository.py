...
class SQLProjectRepositoryAdapter(ProjectRepositoryPort[AsyncSession]):
    async def get_private_key(self, session: AsyncSession, project_id: UUID) -> str | None:
        result = await session.execute(select(Project.private_key).where(Project.id == project_id))
