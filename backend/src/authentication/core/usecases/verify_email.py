...
class VerifyEmailUseCase[SessionType]:
    ...
    async def execute(self, uow: UoWPort[SessionType], email: str, otp: str, client_meta: ClientMetadata | None = None, project_id: UUID | None = None) -> tuple[UserIdentity, str]:
        """
        ...
        if not user:
            await self._logger.warning(f"Verification failed: User {anonymize_email(email)} not found")
            raise InvalidCredentialsException()
            
        if user.is_verified:
            await self._logger.warning(f"Verification failed: User {anonymize_email(email)} is already verified")
            raise InvalidCredentialsException()
        ...
        redis_key = f"pending_reg:{str(project_id)}:{email_hash}" if project_id else f"pending_reg:global:{email_hash}"
        
        exceeded = await self._cache.increment_and_check_exceeds(
            attempt_key, redis_key, verification_settings.OTP_RESEND_WINDOW_SECONDS, verification_settings.OTP_MAX_ATTEMPTS
        )

        if exceeded:
            await self._logger.warning(f"Verification failed: Too many OTP attempts for {anonymize_email(email)}")
        ...
        if not payload:
            await self._logger.warning(f"Verification failed: No pending registration found for {anonymize_email(email)}")
            raise InvalidTokenException()
        ...
        if current_time > otp_expires_at:
            await self._logger.warning(f"Verification failed: OTP expired for {anonymize_email(email)}")
            raise InvalidTokenException()
        ...
        if not verify_otp_hash(provided_otp, stored_otp_hash):
            await self._logger.warning(f"Verification failed: Incorrect OTP for {anonymize_email(email)}")
            raise InvalidTokenException()
