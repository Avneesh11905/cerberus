...
class RefreshSessionUseCase[SessionType]:
    ...
    async def execute(
        ...
        """
        user, new_refresh_token, family_id = await self._refresh_repo.validate(
            uow.session, refresh_token, client_meta=client_meta
        )
        if not user:
            return None, None
        ...
        private_key_override = (
            await self._project_repo.get_private_key(uow.session, user.project_id)
            if user.project_id
            else None
        )
