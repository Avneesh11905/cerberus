...
class RegisterLocalUserUseCase[SessionType]:
    ...
    async def execute(
        ...
        existing = await self._user_repo.find_by_email(uow.session, email, project_id=project_id)
        if existing and existing.is_verified:
            await self._logger.warning(f"Registration failed: Email {anonymize_email(email)} already registered and verified")
        ...
        cache_key = f"pending_reg:{str(project_id)}:{email_hash}" if project_id else f"pending_reg:global:{email_hash}"
        
        # Use SETNX to prevent race conditions when two concurrent registrations for the same email are submitted
        success = await self._cache.set_dict_nx(cache_key, payload, verification_settings.OTP_RESEND_WINDOW_SECONDS)
        if not success:
            await self._logger.warning(f"Registration failed: Concurrent registration attempt for {anonymize_email(email)}")
            raise EmailAlreadyRegisteredException()
        
        # 6. Dispatch email
        await self._email_sender.send_verification_email(email, otp)
        
        await self._logger.info(f"Pending registration cached for {anonymize_email(email)}. Verification OTP sent.")
