...
from src.authentication.api.dependencies import get_current_user, verify_csrf, get_optional_project_id
...
from src.shared.api.utils import extract_client_metadata, generate_csrf_token, set_refresh_token_cookie
from src.shared.config import rate_limit_settings
from src.shared.infrastructure.sql.uow import SQLAlchemyUnitOfWork, get_uow  #  was missing, caused NameError at startup
...
async def register(
    ...
    **Returns:**
    ...
    async with uow:
        await usecase.execute(uow, req.email, req.password, req.name, project_id=project_id, role=UserRole.USER)
...
async def register_tenant(
    ...
    """
    async with uow:
        await usecase.execute(uow, req.email, req.password, req.name, project_id=None, role=UserRole.TENANT)
    return MessageResponse(message="Successfully registered! Please check your email for the 6-digit OTP code.")
...
async def login_local(
    ...
    **Note:** The user must have verified their email address before they can log in.
    ...
    async with uow:
        user, refresh_token, access_token = await usecase.execute(uow, req.email, req.password, client_meta=client_meta, project_id=project_id)
