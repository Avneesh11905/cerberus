...
def test_jwt_rsa_signatures():
    ...
    adapter = JWTAccessTokenAdapter(private_key=global_priv, public_key=global_pub, lifetime_minutes=15)
    
    user = UserIdentity(id=uuid4(), email="test@test.com", role=UserRole.USER, is_verified=True)
    ...
    verified_fail, payload_fail = adapter.verify(token_tenant) # Tries global_pub by default
    assert verified_fail is None
    
    # 3. Sign with Tenant, Verify with Tenant (Should succeed)
    verified_tenant, payload_tenant = adapter.verify(token_tenant, public_key_override=tenant_pub)
