...
async def test_scoped_constraints():
    ...
    hash_x = (
        f"pending_reg:{str(project_x)}:{hashlib.sha256(email.encode()).hexdigest()}"
    )
    hash_y = (
        f"pending_reg:{str(project_y)}:{hashlib.sha256(email.encode()).hexdigest()}"
    )
...
async def test_explicit_tenant_registration_sets_tenant_role():
    ...
    await usecase.execute(
        DummyUoW(), email, "TenantPass1!", "Tenant", role=UserRole.TENANT
    )
