...
@pytest.fixture
def verify_usecase(
    user_repo, logger_port, mock_cache, refresh_token_port, email_sender
):
...
@pytest.fixture
def login_usecase(
    user_repo, refresh_token_port, password_hasher, logger_port, email_sender
):
    access_token = Mock()
...
@pytest.mark.asyncio
async def test_register_and_verify_success(
    register_usecase, verify_usecase, mock_session, user_repo, mock_cache, email_sender
):
...
async def test_register_duplicate_email_fails(
    register_usecase, verify_usecase, mock_session, mock_cache, email_sender
):
...
async def test_login_success(
    ...
    logged_in_user, token, access_token = await login_usecase.execute(
        mock_session, email, "mypassword"
    )
...
async def test_login_unverified_email_returns_403(
    login_usecase, register_usecase, mock_session, email_sender
):
...
@pytest.mark.asyncio
async def test_oauth_only_user_cannot_login_locally(
    login_usecase, mock_session, user_repo
):
...
async def test_email_case_insensitive_login(
    ...
    user, token, access_token = await login_usecase.execute(
        mock_session, email, "SecurePass"
    )
    assert user is not None
