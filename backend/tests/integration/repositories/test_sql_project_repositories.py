...
from src.modules.projects.adapters.project_command_repository import SQLProjectCommandRepositoryAdapter
from src.modules.projects.adapters.project_query_repository import SQLProjectQueryRepositoryAdapter
from src.modules.projects.adapters.project_user_repository import SQLProjectUserRepositoryAdapter
...
@pytest.mark.asyncio
async def test_project_save_and_query(db_session: AsyncSession, project_command_repo, project_query_repo):
...
@pytest.mark.asyncio
async def test_project_delete(db_session: AsyncSession, project_command_repo, project_query_repo):
    tenant = Tenant(email=f"tenant_{uuid4()}@test.com", name="Tenant")
    db_session.add(tenant)
    await db_session.flush()
...
@pytest.mark.asyncio
async def test_project_users_operations(db_session: AsyncSession, project_command_repo, project_user_repo):
    tenant = Tenant(email=f"tenant_{uuid4()}@test.com", name="Tenant")
    db_session.add(tenant)
    await db_session.flush()
    ...
    # Update status
    updated = await project_user_repo.update_user_status(db_session, saved.id, user1.id, is_active=False)
    assert updated is not None
    assert updated.is_active is False

    # Update claims
    updated = await project_user_repo.update_user_claims(db_session, saved.id, user1.id, {"admin": True})
    assert updated is not None
