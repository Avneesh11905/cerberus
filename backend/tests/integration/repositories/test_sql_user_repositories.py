...
from src.modules.auth.authentication.adapters.repository.sql_user_command_repository import (
    SQLUserCommandRepositoryAdapter,
)
from src.modules.auth.authentication.adapters.repository.sql_user_query_repository import (
...
async def test_project_user_oauth(db_session: AsyncSession, command_repo, query_repo):
    ...
    queried = await query_repo.find_by_oauth(
        db_session, provider="google", oauth_sub="12345", project_id=project.id
    )
    assert queried is not None
    assert queried.email == email
...
async def test_link_oauth_account(db_session: AsyncSession, command_repo, query_repo):
    ...
    await command_repo.link_oauth_account(
        session=db_session, user_id=identity.id, provider="github", oauth_sub="gh_123"
    )
    await db_session.flush()

    queried = await query_repo.find_by_oauth(
        db_session, provider="github", oauth_sub="gh_123"
    )
    assert queried is not None
...
async def test_find_by_email(db_session: AsyncSession, command_repo, query_repo):
    ...
    await command_repo.create_user_with_password(
        session=db_session, email=email, name="Find User", password_hash=None
    )
