...
def repo(db_session: AsyncSession, refresh_repo_mock):
    return SQLUserProfileRepositoryAdapter(
        session=db_session, refresh_repo=refresh_repo_mock
    )
...
async def setup_users(db_session: AsyncSession, project: Project):
    ...
    oauth_acc = OAuthAccount(tenant_id=tenant_id, provider="github", oauth_sub="sub456")
...
async def test_get_profile(
    db_session: AsyncSession, repo: SQLUserProfileRepositoryAdapter, setup_users
):
...
async def test_delete_user(
    ...
    repo: SQLUserProfileRepositoryAdapter,
    setup_users,
    refresh_repo_mock,
):
