...
from src.modules.authentication.infrastructure.database.repositories.sql_user_query_repository import SQLUserQueryRepositoryAdapter
...
async def project(db_session: AsyncSession):
    t = Tenant(id=uuid4(), email="tenant_query@example.com", name="Tenant", role=GlobalRole.TENANT, is_active=True, is_verified=True)
    db_session.add(t)
...
@pytest.mark.asyncio
async def test_find_by_email(db_session: AsyncSession, repo: SQLUserQueryRepositoryAdapter, setup_users):
...
@pytest.mark.asyncio
async def test_find_by_id(db_session: AsyncSession, repo: SQLUserQueryRepositoryAdapter, setup_users):
...
@pytest.mark.asyncio
async def test_find_by_oauth(db_session: AsyncSession, repo: SQLUserQueryRepositoryAdapter, setup_users):
...
@pytest.mark.asyncio
async def test_find_password_hash(db_session: AsyncSession, repo: SQLUserQueryRepositoryAdapter, setup_users):
