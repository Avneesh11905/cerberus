...
class MockUserRepository:
    ...
    async def find_by_oauth(self, session, provider: str, oauth_sub: str, project_id: UUID | None = None) -> UserIdentity | None:
    ...
    async def find_by_email(self, session, email: str, project_id: UUID | None = None) -> UserIdentity | None:
    ...
    async def create_user_with_oauth(
        ...
        user_id = uuid4()
        user = UserIdentity(id=user_id, email=email, name=name, picture=cast(AnyHttpUrl, picture) if picture else None, is_verified=True, project_id=project_id, role=role)
        self.users[user_id] = user
        self.oauth_links.append((user_id, provider, oauth_sub))
        return user

    async def link_oauth_account(
        self, session: Any, user_id: UUID, provider: str, oauth_sub: str, project_id: UUID | None = None
    ) -> None:
        self.oauth_links.append((user_id, provider, oauth_sub))

    async def create_user_with_password(
        self, session, email: str, name: str | None, password_hash: str | None, is_verified: bool = False, project_id: UUID | None = None, role: UserRole = UserRole.USER
    ) -> UserIdentity:
        user_id = uuid4()
        user = UserIdentity(id=user_id, email=email, name=name, is_verified=is_verified, project_id=project_id, role=role)
        self.users[user_id] = user
...
class MockEmailSender:
    ...
    async def send_login_detected_email(self, to_email: str, ip_address: str, device_info: str): pass
